...
!> @brief Linear reconstruction of a multidata field from a coarse to fine mesh
!> @details Performs linear reconstruction of a multidata field (W3 or Wtheta) from
...
module sci_prolong_multidata_linear_kernel_mod
...
type, public, extends(kernel_type) :: prolong_multidata_linear_kernel_type
   private
...
contains
  procedure, nopass :: prolong_multidata_linear_kernel_code
end type prolong_multidata_linear_kernel_type

public :: prolong_multidata_linear_kernel_code
...
contains
  ...
  !> @param[in]     stencil_map              Stencil map for coarse field
  !> @param[in]     coarse_mask              Coarse mask
  !> @param[in]     stencil_size_mask        Number of cells covered by the stencil
  !>                                         for the 2D coarse mask
  !> @param[in]     stencil_map_mask         Stencil map for 2D coarse_mask
  ...
  !> @param[in]     ndf_mask                 Num of DoFs per cell for mask
  !> @param[in]     undf_mask                Total num of DoFs on the coarse
  !!                                         grid for mask for this mesh partition
  !> @param[in]     map_mask                 DoFmap of cells on the coarse grid for mask
  subroutine prolong_multidata_linear_kernel_code( nlayers,                 &
                                                   cell_map,                &
    ...
    integer(kind=i_def), intent(in)    :: ndf, ndf_mask
    integer(kind=i_def), intent(in)    :: stencil_size
    integer(kind=i_def), intent(in)    :: stencil_map(ndf, stencil_size)
    integer(kind=i_def), intent(in)    :: stencil_size_mask
    integer(kind=i_def), intent(in)    :: stencil_map_mask(ndf_mask, stencil_size_mask)
    ...
    ! stencil to use)
    if (stencil_size == 5                               .and.   &
        coarse_mask(stencil_map_mask(df,2)) > 0.0_r_def .and.   &
        coarse_mask(stencil_map_mask(df,3)) > 0.0_r_def .and.   &
        coarse_mask(stencil_map_mask(df,4)) > 0.0_r_def .and.   &
        coarse_mask(stencil_map_mask(df,5)) > 0.0_r_def .and.   &
        coarse_mask(stencil_map_mask(df,1)) > 0.0_r_def) then
      ! Only to reconstruction when we are inside mask and have stencil, otherwise
      ! we do low order mapping
        ...
        do x_idx = 1, ncell_fine_per_coarse_x
          do i = 1, ndata
            ...
            do k = 0, top_df
              ...
              coeff_c = coarse_field(map_coarse(df)+k+k_start-1)
      ...
      ! We're at the edge of the domain or mask. Set all fine values to be the coarse
      ...
      do y_idx = 1, ncell_fine_per_coarse_y
        do x_idx = 1, ncell_fine_per_coarse_x
          do i = 1, ndata
            ...
            do k = 0, top_df
              fine_field(map_fine(df,cell_map(x_idx,y_idx))+k+k_start-1) =   &
              coarse_field(stencil_map(df,1)+k+k_start-1)
            end do
          end do
        end do
      end do
  ...
  end subroutine prolong_multidata_linear_kernel_code

end module sci_prolong_multidata_linear_kernel_mod
