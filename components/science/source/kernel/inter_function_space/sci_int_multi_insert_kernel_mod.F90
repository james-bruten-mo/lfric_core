...
module sci_int_multi_insert_kernel_mod
  ...
  type, public, extends(kernel_type) :: int_multi_insert_kernel_type
      private
  ...
  contains
      procedure, nopass :: int_multi_insert_code
  end type

  public :: int_multi_insert_code
...
contains
  ...
  subroutine int_multi_insert_code(nlayers,                    &
                                   output_field,               &
                                   input_field,                &
                                   start_ind,                  &
                                   ndata,                      &
    ...
    integer(kind=i_def), intent(inout) :: output_field(undf_out)

    integer(kind=i_def) :: i

    ! Convert input field to output field
    do i = 0, ndata-1
      output_field(map_out(1)+i+start_ind-1) = int(input_field(map_in(1)+i),i_def)
    end do

  end subroutine int_multi_insert_code

end module sci_int_multi_insert_kernel_mod
