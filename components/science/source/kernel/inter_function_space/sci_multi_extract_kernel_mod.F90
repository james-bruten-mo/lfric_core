...
!-------------------------------------------------------------------------------
!> @brief Extract a field from a section of a multi-data field
!> @details Extract data to a smaller (multi-data) field from the required
!>          section of a larger multi-data field. It is assumed that all
!>          data of the output field will be written to (but not all
!>          elements of the input field will be used).

module sci_multi_extract_kernel_mod
  ...
  type, public, extends(kernel_type) :: multi_extract_kernel_type
      private
  ...
  contains
      procedure, nopass :: multi_extract_code
  end type

  public :: multi_extract_code
...
contains
  ...
  subroutine multi_extract_code(nlayers,                    &
                                output_field,               &
                                input_field,                &
                                start_ind,                  &
                                ndata,                      &
    ...
    do i = 0, ndata-1
      output_field(map_out(1)+i) = input_field(map_in(1)+i+start_ind-1)
    end do

  end subroutine multi_extract_code

end module sci_multi_extract_kernel_mod
