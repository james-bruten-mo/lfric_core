...
contains
  ...
  subroutine eulerian_deppt_code( nlayers,   &
    ...
    do k = 0,nlayers-1
      ...
      dep_pts_x( map_w2(1) + k ) = wind( map_w2(1) + k )*dt
      dep_pts_x( map_w2(3) + k ) = wind( map_w2(3) + k )*dt
      dep_pts_y( map_w2(2) + k ) = -wind( map_w2(2) + k )*dt
      dep_pts_y( map_w2(4) + k ) = -wind( map_w2(4) + k )*dt
    end do
