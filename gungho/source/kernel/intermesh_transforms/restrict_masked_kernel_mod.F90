...
public :: restrict_masked_kernel_code
  ...
  interface restrict_masked_kernel_code
    module procedure  &
      restrict_masked_kernel_code_r_single, &
      restrict_masked_kernel_code_r_double
  end interface
...
contains
  ...
  ! R_SINGLE PRECISION
  ! ==================
  subroutine restrict_masked_kernel_code_r_single( nlayers,                 &
                                                   cell_map,                &
    ...
    real(kind=r_single), dimension(undf_coarse), intent(inout) :: field_coarse
    real(kind=r_single), dimension(undf_fine), intent(in)      :: field_fine
    real(kind=r_single), dimension(undf_fine), intent(in)      :: mask_fine

    integer(kind=i_def) :: df, k, lp_x, lp_y
    real(kind=r_single)    :: denom, non_zero_cells
    ...
    do k = 0, nlayers-1 + (ndf-1)
      field_coarse( dofmap_coarse(df) + k ) = 0.0_r_single
  ...
  end subroutine restrict_masked_kernel_code_r_single

  ! R_DOUBLE PRECISION
  ! ==================
  subroutine restrict_masked_kernel_code_r_double( nlayers,                 &
                                                   cell_map,                &
                                                   ncell_fine_per_coarse_x, &
                                                   ncell_fine_per_coarse_y, &
                                                   ncell_fine,              &
    ...
    real(kind=r_double), dimension(undf_coarse), intent(inout) :: field_coarse
    real(kind=r_double), dimension(undf_fine), intent(in)      :: field_fine
    real(kind=r_double), dimension(undf_fine), intent(in)      :: mask_fine

    integer(kind=i_def) :: df, k, lp_x, lp_y
    real(kind=r_double)    :: denom, non_zero_cells
    ...
    df = 1
    do k = 0, nlayers-1 + (ndf-1)
      field_coarse( dofmap_coarse(df) + k ) = 0.0_r_double
  ...
  end subroutine restrict_masked_kernel_code_r_double
