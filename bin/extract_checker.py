...
MANUAL_FILES = {
    "um": ["um/src/atmosphere/atmosphere_service/include/qsat_mod_qsat.h",
...
def pattern_search(inlist, pattern, mode='whole'):
    ...
    for item in inlist:
        if pattern.lower() in item.lower() and mode == 'whole' or \
                item.lower().startswith(pattern) and mode == 'start':
            output[item] += 1
    return output
...
def print_extract_list(source_repos, extract_list):
    ...
    '''
    print("\n\nCreating an extract list of "
            f"{source_repos} files....")
...
def generate_extract_lists(set_of_copied_files,
                           set_of_compiled_files):
    '''
    Generate a set of files required to be extracted for each repo
    '''
    compiled_files_to_extract = things_only_in_A_AND_B(set_of_copied_files,
                                                set_of_compiled_files)
    ...
    for source_repos in REPOS_LIST:
        match_set = subsetting_by_pattern(compiled_files_to_extract,
                                                  source_repos)
...
def check_for_uncompiled_files(set_of_copied_files,
                               set_of_compiled_files):
    '''
    ...
    copied_not_compiled = things_in_A_but_not_B(set_of_copied_files,
                                                set_of_compiled_files)
    ...
    if num_uncompiled > 0:
        ...
        print(f"There are {num_uncompiled} files copied but not compiled\n"
                "Please remove any uncompiled files\n"
                "See output for the full list", file=sys.stderr)

        for source_repos in REPOS_LIST:
            print(f"    {len(uncompiled_files[source_repos])} in "
                  f"{source_repos}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--suite_name',
                        default = None,
                        help = "Name of suite run " +
                               "(default = lfric_atm-heads-meto-spice-nightly)")
    parser.add_argument('-d', '--suite_dir',
                        default = None,
                        help = "Directory of rose suite")
